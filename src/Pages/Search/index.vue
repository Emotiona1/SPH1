...
export default {
  ...
  beforeMount() {
    // 发请求之前，把接口需要传递参数，进行整理（在给服务器发请求之前，把参数整理好，服务器会返回查询的数据）
    // Object.assign ES6新增语法 合并对象
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
  },
  // mounted 组件挂载完毕执行一次【仅仅只执行一次】
  mounted() {
  ...
  methods: {
    ...
    changeOrder(flag) {
      ...
      // 准备一个新的order属性值
      let newOrder = "";
      // flag == orginFlag 说明点击的就是默认的时候的那个值
      if (flag == originFlag) {

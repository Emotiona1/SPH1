        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转:从A路由跳转到b路由，这里在加入购物车，进行路由跳转之前，发请求把你购买的产品信息通过请求的形式通知服务器，服务器进行相应存储 -->
...
export default {
  ...
  methods: {
    ...
    // 加入购物车
    async addShopCart() {
      // 1.发请求-----将产品加入到数据库中(通知服务器)
      // 当前这里是派发一个action 也向服务器发送请求 判断加入购物车是否成功，进行相应操作
      // this.$store.dispatch("addOrUpdataShopCart", {skuId: this.$route.params.skuid,skuNum: this.skuNum,});
      // 上面这行代码说白了：调用仓库中的addOrUpdataShopCart,这个方法上加asyc 返回的一定是一个Promise
      // 要么成功 要么失败
      try {
        // 如果成功
        // 1.派发action
